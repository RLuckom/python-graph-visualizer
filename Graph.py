...
class Graph(object):
    ...
    def minimum_spanning_tree(self):
        ...
        while len(tree['vertices']) < len(self.vertex_dict):
            ...
            best_edge = None
            best_vertex = None
            vertex_names = [vertex.name for vertex in tree['vertices']]
            for vertex in tree['vertices']:
                for edge in vertex.edges:
                    if edge not in vertex_names and (vertex.edges[edge] < best_edge_number or best_edge == None):
                        best_edge_number = vertex.edges[edge]
                        best_edge = self.edge_dict[vertex.name, edge]
                        best_vertex = edge
